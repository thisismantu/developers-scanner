<x-app-layout>
    ...
    <div class="row">
        ...
        <div class="col text-end">
            ...
            <x-responsive-nav-link :href="route('companies.create')" :active="request()->routeIs('companies')" class="btn btn-primary btn-sm">
                        ...
                        <table class="table table-striped table-sm datatable2 mt-3">
                            ...
                            <tbody>
                                @foreach ($response as $dataLine)
                                    <form method="post" action="{{ Route('companies.destroy', $dataLine->id) }}">
                                        @method('DELETE')
                                        @csrf
                                        <tr>
                                            ...
                                            <td>{{ $dataLine->name }}</td>
                                            <td><span
                                                class="badge bg-{{ $dataLine->is_active == 1 ? 'success' : 'danger' }}">{{ $dataLine->is_active == 1 ? 'Active' : 'Inactive' }}</span>
                                            </td>
                                            <td>{{ $dataLine->user->name }}</td>
                                            ...
                                            <td><button type="submit"
                                                ...
                                                <a href="{{ Route('companies.edit', $dataLine->id) }}"><i
